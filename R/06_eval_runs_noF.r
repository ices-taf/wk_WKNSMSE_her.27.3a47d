...
for(idxHCR in 1:length(eval.case)){
  ...
  for(idxFOption in 1:length(eval.options)){
    ...
    for(idxMetric in 1:length(eval.period)){
      ...
      F26FieldName    <- paste0('F26_',eval.period[[idxMetric]][1])
      F01FieldName    <- paste0('F01_',eval.period[[idxMetric]][1])
      ...
      eval.table[idxTable,F26FieldName] <- mean(f26Quant['50%',])
      
      # F01
      FHCR01      <- FHCR[f01,metricsPeriod]
      
      f01Mat <- apply(drop(FHCR01[,metricsPeriod]),c(2,3),'mean')
      
      f01Quant <- apply(f01Mat, 1, quantile, probs=c(0.05, 0.5, 0.95), na.rm=TRUE)
      
      eval.table[idxTable,F01FieldName] <- mean(f01Quant['50%',])
    }
    
  }
}

write.table(eval.table,
            file.path(outPath,paste0(outputName,'_',nits,'its.csv')),
            sep = ",",
            row.names = FALSE)
